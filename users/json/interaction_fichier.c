...
int ajout_ligne_utilisateur_et_fichier_historique(char *login,char *password){
    char nom_fichier[40]="./historique/";
    strcat(nom_fichier,login);
    strcat(nom_fichier,".json");
    FILE *temp=fopen("temp_users.json","w");
    FILE *ancien=fopen("users.json","r");
    ...
    if (temp!=NULL && ancien !=NULL && new_file_histo!=NULL){
        ...
        remove("users.json");
        rename("temp_users.json","users.json");
        return 1;
...
int creation_ressource(char *nom,char *description,int type,char *login){
    FILE *f_ressource=fopen("ressources.json","r");
    FILE *temp=fopen("temp_ressources.json","w");
    if (temp!=NULL && f_ressource!=NULL){
        ...
        fclose(temp);
        remove("ressources.json");
        rename("temp_ressources.json","ressources.json");
...
int recherche_occurence_login(char *login){
    FILE *fichier_users=fopen("users.json","r");
...
int supprimer_ligne_users(char *login){
    FILE *fichier_users=fopen("users.json","r");
    FILE *f_temp=fopen("temp_users.json","w");
    char motRech[40]="\"login\": \"";
    strcat(motRech,login);
    motRech[strlen(motRech)]='\"';
    motRech[strlen(motRech)]='\0';
...
int supprimer_ligne_ressource(char *id){  //transformer id en int peut etre ?
    FILE *fichier_ressources=fopen("ressources.json","r");
    FILE *f_temp=fopen("temp_ressources.json","w");
    char motRech[40]="\"id\": \"";
    strcat(motRech,id);
    motRech[strlen(motRech)]='\"';
    motRech[strlen(motRech)]='\0';
